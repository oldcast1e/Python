...
def getArrivalInfo(arsId):
    try:
        ...
        bus_info_str = f"Station Name: Sejong Univ.\n\n"
        for item in items:
...
def toggle_display():
    ...
    while True:
        with lock:
            ...
            print(f"Current display: {current_display}")  # Log the current display state
        time.sleep(10)  # Change every 60 seconds
...
def bus_info_route():
    with lock:
        if current_display == "bus":
            ...
            </html>''', weather_info=weather_info)
