...
def getArrivalInfo(arsId):
    try:
        ...
        # 버스 정보를 저장할 문자열 변수
        bus_info_str = f"정류소명: {station_name}<br>"
        ...
        for item in items:
            ...
            if predict_msg == '운행종료':
                bus_info_str += f'{bus_route_name}: 운행종료!<br>'
            elif predict_msg == '곧 도착':
                bus_info_str += f'{bus_route_name}: 곧 도착!<br>'
            else:
                ...
                bus_info_str += f'{bus_route_name}: {minutes.strip()} 분 {seconds} 후!<br>'
